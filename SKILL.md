...
version: "22.0"
description: 从合法免费来源搜索、下载电子书并转换为Markdown存入Obsidian知识库；支持PDF/EPUB提取文本、HTML在线书籍抓取、版权判断、Obsidian frontmatter生成、笔记框架创建；内置书源注册表（20+已验证书籍）和Gutenberg在线搜索；v22安全加固：EPUB Zip Bomb/路径穿越防护、SSRF IPv6+DNS重绑定防护、Markdown注入防护、HTML深度限制、Cookie隔离、SSL白名单、Content-Type校验；当用户提到找书、下载电子书、存入知识库、找编程书/AI书/写作书时触发
---
...
> **v22.0** — 安全加固版。基于v21.0的SBI评测优化，额外修复12个安全漏洞：EPUB Zip Bomb防护、EPUB路径穿越防护、SSRF IPv6绕过修复、DNS重绑定防护、Markdown注入防护、HTML解析器深度限制、临时文件安全、Content-Type校验、Cookie隔离、全局资源限制、SSL降级白名单、连接后SSRF二次验证。
...
| EPUB含Zip Bomb | 拦截：文件数>500/单文件>50MB/总大小>500MB → 截断 |
| EPUB路径穿越 | 拦截：含../或绝对路径的ZIP文件名 → 跳过该文件 |
| SSRF攻击 | 拦截：私有IP/非HTTP协议/IPv6私有/DNS重绑定 → NetworkError |
| 可执行文件下载 | 拦截：Content-Type为exe/sh/bat → 拒绝下载 |
| 恶意HTML深度攻击 | 拦截：嵌套>256层 → 跳过深层内容 |
| SSL证书异常 | 仅白名单域名(pages.cs.wisc.edu等)允许降级 |

## 🔒 安全防护清单（v22）

> 以下安全机制自动生效，Agent无需手动调用。

| # | 威胁 | 防护机制 | 严重度 |
|---|------|---------|--------|
| 1 | EPUB Zip Bomb | MAX_EPUB_FILES=500, MAX_EPUB_SINGLE_FILE=50MB, MAX_EPUB_TOTAL_SIZE=500MB | CRITICAL |
| 2 | EPUB路径穿越 | normpath检查+拒绝../和绝对路径 | CRITICAL |
| 3 | SSRF IPv6绕过 | DNS解析同时检查AF_INET+AF_INET6 | CRITICAL |
| 4 | SSRF DNS重绑定 | _check_connected_ip()连接后验证对端IP | CRITICAL |
| 5 | Markdown注入 | _md_escape()转义[]()/#等MD特殊字符 | CRITICAL |
| 6 | HTML栈溢出 | HTMLToMarkdown MAX_NESTING_DEPTH=256 | CRITICAL |
| 7 | 临时文件竞态 | mkdtemp()不可预测路径替代固定/tmp/book_dl | CRITICAL |
| 8 | 可执行文件下载 | _validate_content_type()拒绝application/x-executable等 | HIGH |
| 9 | Cookie泄露 | cookies={}隔离沙箱请求 | HIGH |
| 10 | 资源耗尽 | MAX_DOWNLOAD_SIZE=200MB + MAX_TOTAL_DOWNLOADS=2GB | HIGH |
| 11 | MITM中间人 | SSL降级仅限SSL_SKIP_DOMAINS白名单 | HIGH |
| 12 | YAML注入 | _yaml_escape()转义换行/三横线/引号 | 已有(v18) |
